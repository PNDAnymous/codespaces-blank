...
enum class colType { Boolean, Int, Float, Date, Text };
colType infer_type(const vector<string>& samples) {
    ...
    for (const auto& val : samples) {
        ...
        if (!regex_match(val, regex("^-?\\d+$"))) all_int = false;
        if (!regex_match(val, regex("^-?\\d*\\.\\d+$"))) all_float = false;
        if (!regex_match(val, regex("^(true|false|yes|no|0|1)$", regex::icase))) all_bool = false;
        if (!regex_match(val, regex("^\\d{4}-\\d{2}-\\d{2}$"))) all_date = false;
    }

    if (all_bool) return colType::Boolean;
    if (all_int) return colType::Int;
    if (all_float) return colType::Float;
    if (all_date) return colType::Date;
...
int main() {
    ...
    try {
        ...
        for (size_t i = 0; i < headers.size(); ++i) {
            ...
            if (i < headers.size() - 1) theQuerier += ", ";
        }
        theQuerier += ");";
